...
**Core Components**:
- `runCmds([]*exec.Cmd)`: Chains commands via pipes (existing function, preserved)
- `getPids([]string)`: Parses command output into PID list (existing function, preserved)
...
2. Construct command pipeline: `ps aux | grep <keyword> | grep -v grep | awk '{print $2}'`
...
- Preserve existing `runCmds()` and `getPids()` functions
